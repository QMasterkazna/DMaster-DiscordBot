...
class Music(commands.Cog):
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Музыку сейчас не слушаю...')
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Пропустить голосование добавлено, в настоящее время на **{}/3**'.format(total_votes))

        else:
            await ctx.send('Вы уже проголосовали за пропуск этой песни.')
